    ...
    Addr pf_addr = (region_num << rs_shift) + (Addr(region_offset) << bs_shift);
        ...
        if (has_trigger) {
            addresses.emplace_back(AddrPriority(pf_addr, prio,
                trigger_info.pfSourceType == PrefetchSourceType::PF_NONE ?
                    pfSourceType : trigger_info.pfSourceType,
                trigger_info));
        } else {
                       ...
                       const TriggerInfo *trigger)
{
    ...
    if (e) {
        if (e->region_addr != region_addr) {
            DPRINTF(HWPrefetch, "Warning: Insert called with existing entry "
                                "but different region_addr. existing=%#lx "
                                "new=%#lx\n",
...
PrefetchFilter::hasPFRequestsInBuffer()
{
    ...
    if (n == 0)
        return false;
    DPRINTF(HWPrefetch,
            "hasPFRequestsInBuffer called. table size: %lu,name=%s\n",
