        ...
        if next_ctx is None:
            ...
            if len(sample_examples[next_cls]) < 3:
                sample_examples[next_cls].append(
                    ...
                    f"next@{next_ctx.tick} "
                    f"main=({next_ctx.main_table},{next_ctx.main_index},"
                    f"{next_ctx.main_tag},pc={fmt_pc(next_ctx.main_entry_pc)}) "
                    f"alt=({next_ctx.alt_table},{next_ctx.alt_index},"
                    f"{next_ctx.alt_tag},pc={fmt_pc(next_ctx.alt_entry_pc)}) "
                    f"fh={next_ctx.index_fh}"
                )
