...
XSCompositePrefetcher::XSCompositePrefetcher(const XSCompositePrefetcherParams &p)
    : Queued(p),
      ...
      stridestream_pfFilter_l2l3(
          p.stridestream_L2L3_filter_indexing_policy,
          p.stridestream_L2L3_filter_replacement_policy,
    ...
    if (pht_entry) {
        ...
        if (found){
            ...
            phtSentPrefetch[0] = phtsentInfo(
                region_addr, region_bit_cur, 0, true,
                pht_entry->decr_mode, secure, phtPFLevel,
                &pfi.trigger_info);
        ...
        if (found){
            ...
            phtSentPrefetch[1] = phtsentInfo(
                region_inc_addr, region_bit_inc, 0, true,
                pht_entry->decr_mode, secure, phtPFLevel,
                &pfi.trigger_info);
        ...
        if (found){
            ...
            phtSentPrefetch[2] = phtsentInfo(
                region_dec_addr, region_bit_dec, 0, true,
                pht_entry->decr_mode, secure, phtPFLevel,
                &pfi.trigger_info);
