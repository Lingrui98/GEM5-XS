...
MicroTAGE::getBranchIndexInBlock(Addr branchPC, Addr startPC) {
    ...
    if (position >= maxBranchPositions) {
        warn_once("MicroTAGE: branch %#lx exceeds block [%#lx, %#lx) "
                  "(blockSize=%lu, instShift=%u, maxPositions=%u); "
                  "clamping index",
                  branchPC, alignedPC,
                  alignedPC + blockSize,
...
MicroTAGE::TageStats::TageStats(statistics::Group* parent, int numPredictors, int numBanks):
    ...
    ADD_STAT(updateUtageHit, statistics::units::Count::get(),
             "number of updates where utage provided the main prediction"),
    ADD_STAT(updateUtageHitWrong, statistics::units::Count::get(),
             "number of updates where utage prediction was wrong"),
