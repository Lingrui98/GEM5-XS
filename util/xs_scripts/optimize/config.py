...
def parse_param_space(param: dict):
    ...
    if space_type == "integer":
        ...
        if min_val > max_val:
            raise ValueError(
                f"'min_int' ({min_val}) must be less than or equal to "
                f"'max_int' ({max_val}) for parameter '{name}'"
            )
        ...
        if min_val > max_val:
            raise ValueError(
                f"'min_float' ({min_val}) must be less than or equal to "
                f"'max_float' ({max_val}) for parameter '{name}'"
            )
        ...
        if min_exp > max_exp:
            raise ValueError(
                f"'min_exp' ({min_exp}) must be less than or equal to "
                f"'max_exp' ({max_exp}) for parameter '{name}'"
            )
